...
class GitHubAnalytics {
  ...
  async track(data) {
    ...
    try {
      ...
      const issueBody = `## 🧞‍♀️ TestGenie Analytics Event [${eventId}]
...
**Version**: ${data.version || require(require('path').join(__dirname, '../package.json')).version}
...
---
*Auto-generated by TestGenie CLI v${data.version || require(require('path').join(__dirname, '../package.json')).version}*
*Event logged on ${timestamp} for permanent business analytics*`;

      const issue = {
        title: `📊 Analytics: ${data.action} by ${userData.username} [${eventId}]`,
        body: issueBody,
        labels: [
          'analytics', 
          data.action, 
      ...
      if (response.ok) {
        ...
        console.log(`📊 Analytics tracked: ${data.action} [${eventId}] → Issue #${result.number}`);
        ...
        console.log(`📊 Analytics: GitHub API responded with ${response.status}`);
      }
    } catch (error) {
      // Silent fail for analytics - never break functionality
      console.log(`📊 Analytics: Unable to connect (${error.message})`);
    }
  }
  ...
  async scheduleDataArchival() {
    ...
    console.log(`📦 Analytics: Checking for data to archive (older than ${cutoffDate.toISOString().split('T')[0]})`);
  }
